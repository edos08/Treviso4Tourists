...
@IBDesignable open class Card: UIView, CardDelegate {
     ...
     */
    @IBInspectable public var CTVController: UIViewController = UIViewController() {
        didSet{
            CTViewController = CTVController
        }
    }
    /**
     MainViewController.
     */
    @IBInspectable var IconImage: UIImageView = UIImageView() {
        didSet{
            IconImg = IconImage
        }
    }
    /**
     ID for the Card.
     */
    @IBInspectable public var cardID: Int = 0 {
        didSet{
            cardIDINT = cardID
        }
    }
    ...
    var cardIDINT:Int = 0
    var CTViewController: UIViewController = UIViewController()
    ...
    @objc func cardTapped() {
        print("toccato " + String(cardIDINT))
        globalCardID = cardIDINT
...
extension Card: UIGestureRecognizerDelegate {
    ...
    open override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if !inHome {
            ...
            CTViewController.navigationController?.pushViewController(newVC, animated: true)
            /*UIView.animate(withDuration: 0.2, animations: {
                self.backgroundIV.transform = CGAffineTransform(scaleX: 0.90, y: 0.90)
            }) { _ in
                UIView.animate(withDuration: 0.1, animations: {
                    self.backgroundIV.transform = CGAffineTransform.identity
                })
            }*/
        } else {
...
open class getID {
    ...
    public func passID() -> Int {
        return globalCardID
    }
}

open class getDetailViewController {
    
    public init() {}
    
    public func passDetailViewController() -> UIViewController {
        return globalDetailViewController
    }
}
